#!/usr/bin/env python3
"""
Find and analyze signSuccess method
"""
import struct
import sys
...
for i in range(0, text_size - 8, 4):
    ...
    if (insn2 & 0x1F) == rd or ((insn2 >> 5) & 0x1F) == rd:
        ...
        if target == signsuccess_sel:
            signsuccess_ref = i
            print(f"[+] Found signSuccess reference at: {text_vaddr + i:#x}")
            break

if signsuccess_ref is None:
    print("[!] signSuccess not found")
    sys.exit(1)

func_start = None
for j in range(signsuccess_ref, max(0, signsuccess_ref - 4096), -4):
...
print(f"[+] signSuccess function at: {func_addr:#x}")
print(f"[+] Function size estimate: {(signsuccess_ref - func_start) + 100} bytes")
print()
