#!/usr/bin/env python3
"""
Find selectVC getter method and create a patch that:
1. Calls [self selectVC] to get the object
2. Calls [result signSuccess]
"""
import struct
import sys
...
print("[+] Searching for selectVC property getter...")

# Find all methods in DASignProcessVC
                            ...
                            if vaddr2 <= sel_vm < vaddr2 + size2:
                                ...
                                if sel_end > sel_foff:
                                    ...
                                    if sel_name == 'selectVC':
                                        selectvc_getter = imp_vm
                                        print(f"[+] Found selectVC getter: IMP={imp_vm:#x}")
                                        break
                                break
                    break
        if selectvc_getter:
...
if not selectvc_getter:
    print("[!] selectVC getter not found in method table")
    print("[!] Property might be synthesized or in parent class")
    print("[!] Cannot create binary patch without getter address")
    sys.exit(1)

print(f"\n[SUCCESS] selectVC getter at {selectvc_getter:#x}")
print("Next step: Create patch that calls this getter then signSuccess")
