#!/usr/bin/env python3
"""
Advanced patch: Call [self.selectVC signSuccess] using objc_msgSend
"""
import struct
import sys
import os
...
if not selectvc_sel or not signsuccess_sel or not installclick_sel:
    print(f"[!] Selectors not found")
    print(f"    selectVC: {selectvc_sel}")
    print(f"    signSuccess: {signsuccess_sel}")
    print(f"    installClick: {installclick_sel}")
    sys.exit(1)

print(f"[+] selectVC selref: {selectvc_sel:#x}")
