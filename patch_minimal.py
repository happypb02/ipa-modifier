#!/usr/bin/env python3
"""
Keep original installClick intact, just NOP it out
Find the function and replace ALL instructions with NOPs, then add RET at start
"""
import struct
import sys
import os
...
# Read the prologue to understand stack frame
prologue = r32(data, text_foff + func_start)
print(f"[+] Prologue: {prologue:#010x}")

# Proper minimal function:
# 1. Save FP/LR (keep original prologue)
# 2. Restore FP/LR (matching epilogue)
# 3. RET

# Keep the original prologue, then add epilogue + RET
# Original: STP X29, X30, [SP, #-XX]!
# We need: LDP X29, X30, [SP], #XX
# Then: RET

# Extract the immediate from prologue
imm = ((prologue >> 15) & 0x7F)
if imm & 0x40:
    imm = imm | 0xFFFFFF80  # Sign extend

# Create epilogue
epilogue = 0xA8C00000 | ((imm & 0x7F) << 15) | (30 << 10) | (29 << 5) | 31  # LDP X29, X30, [SP], #imm

# Write: prologue (keep), epilogue, RET
w32(data, text_foff + func_start + 4, epilogue)
w32(data, text_foff + func_start + 8, 0xD65F03C0)  # RET

with open(binary_path, "wb") as f:
    f.write(data)

print("[+] Patched: installClick is now minimal (prologue + epilogue + RET)")
print("[+] Button will do nothing, no crash")
